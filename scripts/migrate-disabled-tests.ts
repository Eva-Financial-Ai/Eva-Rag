...
interface TestFile {
  ...
  status: 'disabled' | 'migrated' | 'failed';
}
...
const testPriorities: Record<string, 'high' | 'medium' | 'low'> = {
...
async function auditDisabledTests(): Promise<TestFile[]> {
...
async function migrateTestFile(testFile: TestFile): Promise<boolean> {
  try {
    ...
    let updatedContent = content
      ...
      .replace(/\bjest\./g, 'vi.')
      .replace(/\bjest\(/g, 'vi(')
      // Update expect syntax
      .replace(/expect\.extend/g, '// expect.extend')
    ...
    console.error(`❌ Failed to migrate ${testFile.name}:`, error);
    return false;
  }
}

async function generateMigrationReport(testFiles: TestFile[]) {
  const report = {
    ...
    migrated: testFiles.filter(f => f.status === 'migrated').length,
