...
export class WorkflowAutomationService {
  ...
  private calculateAllFinancialRatios(financialData: Record<string, unknown>): FinancialRatios {
    // Implementation of financial ratio calculations with proper type casting
    const currentAssets = Number(financialData?.currentAssets) || 0;
    const currentLiabilities = Number(financialData?.currentLiabilities) || 0;
    const cash = Number(financialData?.cash) || 0;
    const inventory = Number(financialData?.inventory) || 0;
    const totalAssets = Number(financialData?.totalAssets) || 0;
    const totalLiabilities = Number(financialData?.totalLiabilities) || 0;
    const totalEquity = Number(financialData?.totalEquity) || 0;
    const revenue = Number(financialData?.revenue) || 0;
    const grossProfit = Number(financialData?.grossProfit) || 0;
    const netIncome = Number(financialData?.netIncome) || 0;
    const interestExpense = Number(financialData?.interestExpense) || 0;
    const ebit = Number(financialData?.ebit) || 0;
    ...
    return {
      ...
      debtServiceCoverageRatio:
        Number(financialData?.totalDebtService) > 0
          ? (netIncome + interestExpense) / Number(financialData.totalDebtService)
          : 0,

      // Efficiency Ratios
      assetTurnover: totalAssets > 0 ? revenue / totalAssets : 0,
      inventoryTurnover: inventory > 0 ? Number(financialData?.costOfGoodsSold) / inventory : 0,
      receivablesTurnover:
        Number(financialData?.accountsReceivable) > 0
          ? revenue / Number(financialData.accountsReceivable)
          : 0,
    };
  }
  ...
  private async performCashFlowAnalysis(
  ...
  private async performGAAPComplianceCheck(financialData: Record<string, unknown>): Promise<any> {
  ...
  private async generateDSCRReport(
