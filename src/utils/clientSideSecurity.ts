...
export class DataProtector {
   ...
   */
  static maskSensitiveData(data: Record<string, unknown>, maskConfig: SensitiveDataMask = {}): any {
    ...
    Object.keys(masked).forEach(key => {
      ...
      if (maskConfig.ssn !== false && (lowerKey.includes('ssn') || lowerKey.includes('social'))) {
        masked[key] = this.maskSSN(String(value));
      }
      ...
      // Credit card masking
        ...
        masked[key] = this.maskCreditCard(String(value));
      }

      // Email masking
      else if (maskConfig.email !== false && lowerKey.includes('email')) {
        masked[key] = this.maskEmail(String(value));
      }

      // Phone masking
      else if (maskConfig.phone !== false && lowerKey.includes('phone')) {
        masked[key] = this.maskPhone(String(value));
      }

      // Recursive masking for nested objects
      else if (typeof value === 'object' && value !== null) {
        masked[key] = this.maskSensitiveData(value as Record<string, unknown>, maskConfig);
      }
    });
...
class ConsoleProtector {
  ...
  private containsSensitiveData(str: string): boolean {
    const sensitivePatterns = [
      ...
      /\b\d{_10,17}\b/, // Account number pattern
...
export class ClientSecurityManager {
   ...
   */
  public maskDataForDisplay(data: Record<string, unknown>, config?: SensitiveDataMask): any {
